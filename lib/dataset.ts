...
/** Pick the column that holds the order quantity (الكميه), used as the editable field. */
export function detectQuantityColumn(
  columns: string[],
  numericColumns: string[]
): string | null {
  const byName = columns.find((c) => /كمي|quantit|qty/i.test(c));
