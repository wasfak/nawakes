...
/** Drop junk columns (xlsx `__EMPTY*`) and any column that is empty for every row. */
...
export default function UploadPage() {
  ...
  // Save the full uploaded dataset to the database, then it shows on the home page.
  const send = async () => {
    setSending(true);
    try {
      // Keep only detected columns; store numeric columns (الكميه/السعر/خصم اساسى …)
      // as real numbers so we can run calculations on them later.
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5 p-6">
      ...
      {hasData && (
        <DataTable columns={columns} rows={dataRows} numericColumns={numericCols} />
      )}
    </div>
  );
}
